...
def reference_summary(result):
    ...
    if provenance.get("reference_mode") != "registered_urls":
        return "ローカルAIのみ（登録URL参照なし）"
    catalog = provenance.get("reference_catalog") or []
    image_count = sum(len(group.get("images", [])) for group in catalog)
    return f"登録URL参照: {provenance.get('reference_profile', '')} / {len(catalog)}キャラクター / {image_count}画像"


def reference_urls(result):
    provenance = result.get("provenance") or {}
    values = []
    for group in provenance.get("reference_catalog") or []:
...
def export_json(rows, destination):
    atomic_text(destination, json.dumps({"application": "Character Lens", "version": VERSION, "exported": now(), "notice": "通常はローカルAIのみ。登録URL参照を使用した結果にはプロファイル・URL・取得時情報を記録し、参照画像自体は埋め込みません。", "images": rows}, ensure_ascii=False, indent=2, allow_nan=False))


def export_csv(rows, destination):
    output = io.StringIO(newline="")
    writer = csv.writer(output)
    writer.writerow(["ファイル名", "元画像パス", "状態", "候補順位", "キャラクター候補", "作品", "推定類似度（確率ではない）", "評価できた重み（%）", "確かさ（自己申告）", "一致点", "相違点", "理由", "モデル", "参照プロファイル", "参照URL", "人の修正名", "確認済み", "確認メモ", "エラー", "専用モデルの候補と分類出力"])
    for row in rows:
...
def export_html(rows, destination):
    ...
    for row in rows:
        ...
        details.append(f'<p class="muted">参照: {esc(reference_summary(result))}</p>')
        if reference_urls(result):
            details.append(f'<p class="muted">参照URL: {esc(reference_urls(result))}</p>')
